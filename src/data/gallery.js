import gallery1 from "../assets/gallery/gallery1.webp";
import gallery2 from "../assets/gallery/gallery2.webp";
import gallery3 from "../assets/gallery/gallery3.webp";
import gallery4 from "../assets/gallery/gallery4.webp";
import gallery5 from "../assets/gallery/gallery5.webp";
import gallery6 from "../assets/gallery/gallery6.webp";
import gallery7 from "../assets/gallery/gallery7.jpg";
import gallery8 from "../assets/gallery/gallery8.jpg";
import gallery9 from "../assets/gallery/gallery9.jpg";
import gallery10 from "../assets/gallery/gallery10.jpg";
import gallery11 from "../assets/gallery/gallery11.jpg";
import gallery12 from "../assets/gallery/gallery12.jpg";
import gallery13 from "../assets/gallery/gallery13.jpg";
import gallery14 from "../assets/gallery/gallery14.webp";
import gallery15 from "../assets/gallery/gallery15.webp";
import gallery16 from "../assets/gallery/gallery16.jpg";
import gallery17 from "../assets/gallery/gallery17.jpg";
import gallery18 from "../assets/gallery/gallery18.webp";
import gallery19 from "../assets/gallery/gallery19.webp";
import gallery20 from "../assets/gallery/gallery20.webp";
import gallery21 from "../assets/gallery/gallery21.jpg";
import gallery22 from "../assets/gallery/gallery22.jpg";
import gallery23 from "../assets/gallery/gallery23.jpg";
import gallery24 from "../assets/gallery/gallery24.webp";
import gallery25 from "../assets/gallery/gallery25.jpg";
// import gallery26 from "../assets/gallery/gallery26.jpg";
// import gallery27 from "../assets/gallery/gallery27.jpg";
export default [
  { img: gallery1, name: "Elegant Wedding Portrait",  category: "Wedding" },
  { img: gallery2, name: "Romantic Pre-Wedding Shoot", category: "Pre-Wedding" },
  { img: gallery3, name: "Candid Wedding Moments", category: "Wedding" },
  { img: gallery4, name: "Engagement Celebration", category: "Engagement" },
  { img: gallery5, name: "Luxury Bridal Shoot", category: "Wedding" },
  { img: gallery6, name: "Outdoor Couple Session", category: "Pre-Wedding" },
  { img: gallery7, name: "Traditional Wedding Rituals", category: "Wedding" },
  { img: gallery8, name: "Elegant Wedding Portrait", category: "Wedding" },
  { img: gallery9, name: "Romantic Pre-Wedding Shoot", category: "Pre-Wedding" },
  { img: gallery10, name: "Candid Wedding Moments", category: "Wedding" },
  { img: gallery11, name: "Engagement Celebration", category: "Engagement" },
  { img: gallery12, name: "Luxury Bridal Shoot", category: "Wedding" },
  { img: gallery13, name: "Outdoor Couple Session", category: "Pre-Wedding" },
  { img: gallery14, name: "Traditional Wedding Rituals", category: "Wedding" },
  { img: gallery15, name: "Elegant Wedding Portrait", category: "Wedding" },
  { img: gallery16, name: "Romantic Pre-Wedding Shoot", category: "Pre-Wedding" },
  { img: gallery17, name: "Candid Wedding Moments", category: "Wedding" },
  { img: gallery18, name: "Engagement Celebration", category: "Engagement" },
  { img: gallery19, name: "Luxury Bridal Shoot", category: "Wedding" },
  { img: gallery20, name: "Outdoor Couple Session", category: "Pre-Wedding" },
  { img: gallery21, name: "Traditional Wedding Rituals", category: "Wedding" },
  { img: gallery22, name: "Elegant Wedding Portrait", category: "Wedding" },
  { img: gallery23, name: "Romantic Pre-Wedding Shoot", category: "Pre-Wedding" },
  { img: gallery24, name: "Candid Wedding Moments", category: "Wedding" },
  { img: gallery25, name: "Engagement Celebration", category: "Engagement" },
  // { img: gallery26, name: "Luxury Bridal Shoot", category: "Wedding" },
  // { img: gallery27, name: "Luxury Bridal Shoot", category: "Wedding" },
];
